...
def build_classification_system_prompt(config: ClientConfig) -> str:
    ...
    return f"""You are a content analyst. You will receive a batch of news/intelligence items. \
For each item, score it for every edition listed below and extract structured facts.
...
- so_what: 1–2 sentences on why this matters to the edition's audience.
- now_what: 1–2 sentences on what action or response the audience should consider.
...
- confidence_note: brief note if you are uncertain about any assessment. Null if confident.

Do NOT invent facts not present in the source text. Analyze ALL items in the batch.
""".strip()
...
- theme_of_week: one sentence capturing the dominant theme across this run's most important items.
- editors_note: 2–3 sentences of editorial commentary on the pattern, risk, or opportunity.
- whats_new_digest: 3–5 sentences summarizing what is genuinely new or changed.

Be concise, authoritative, and specific. Do not repeat item titles verbatim.
""".strip()
